...
import { useGameArticlesList, useDeleteGameArticle } from "@/hooks/useGameArticles";
...
export default function GameArticlesPage() {
  ...
  const { data, isLoading } = useGameArticlesList({
    page: page + 1,
    limit: PAGE_SIZE,
    status: status || undefined,
    q: search || undefined,
  });
  const deleteArticle = useDeleteGameArticle();
  const [deletingId, setDeletingId] = useState<string | null>(null);
  const [deleteConfirm, setDeleteConfirm] = useState<{ id: string; title: string } | null>(null);

  const handleDeleteClick = (article: GameArticle) => {
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      renderCell: (params) => (
        <Box>
          <IconButton size="small" component={Link} href={`/game-articles/${params.row.slug}`} aria-label="View">
            <VisibilityIcon fontSize="small" />
          </IconButton>
          {canEditOwnedContent(user?.role, user?.id, params.row.createdBy?.id) && (
            <>
              <IconButton size="small" component={Link} href={`/game-articles/${params.row.slug}/edit`} aria-label="Edit">
  ...
  return (
    <Box sx={{ width: "100%", minWidth: 0 }}>
      <PageHeader
        title="Game Articles"
        subtitle="Manage game articles and content."
        action={<Button component={Link} href="/game-articles/new" variant="contained" color="primary" startIcon={<AddIcon />}>Add Article</Button>}
      />
