...
import { useGameArticleBySlug } from "@/hooks/useGameArticles";
...
export default function GameArticleViewPage() {
  const params = useParams();
  const slug = params.slug as string;
  const { data: article, isLoading } = useGameArticleBySlug(slug);
  const { user } = useAuth();
  ...
  if (isLoading || !article) {
    return (
      <Box>
        <Button startIcon={<ArrowBackIcon />} component={Link} href="/game-articles" sx={{ mb: 2 }}>Back</Button>
  ...
  return (
    <Box sx={{ width: "100%", minWidth: 0 }}>
      <Box sx={{ display: "flex", flexDirection: { xs: "column", sm: "row" }, justifyContent: "space-between", alignItems: { xs: "stretch", sm: "flex-start" }, mb: 2, gap: 2 }}>
        <Button startIcon={<ArrowBackIcon />} component={Link} href="/game-articles">Back to Articles</Button>
        {canEdit && (
          <Button component={Link} href={`/game-articles/${article.slug}/edit`} variant="contained" color="primary" startIcon={<EditIcon />}>Edit</Button>
        )}
      </Box>
