...
import {
  useCasinoArticlesList,
  useCasinoArticle,
  useUpdateCasinoArticle,
} from "@/hooks/useCasinoArticles";
...
export default function EditCasinoArticlePage() {
  const params = useParams();
  const router = useRouter();
  const slug = params.slug as string;
  const { data: listData, isLoading: listLoading } = useCasinoArticlesList({
    q: slug,
    limit: 50,
  });
  ...
  const { data: article, isLoading: articleLoading } = useCasinoArticle(id);
  const update = useUpdateCasinoArticle(article?.id ?? "");
  ...
  if (!listLoading && listData && articleBySlug == null) {
    return (
      <Box>
        <Button component={Link} href="/casino-articles" sx={{ mb: 2 }}>
          Back
        </Button>
  ...
  return (
    <ArticleEditorLayout
      title="Edit Article"
      subtitle={subtitle}
      backHref={`/casino-articles/${article.slug}`}
      backLabel="Back to Article"
      submitLabel="Save changes"
      cancelHref={`/casino-articles/${article.slug}`}
