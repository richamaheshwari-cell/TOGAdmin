...
import { useCasinoArticlesList, useDeleteCasinoArticle } from "@/hooks/useCasinoArticles";
import { useAuth } from "@/contexts/AuthContext";
import type { CasinoArticle } from "@/lib/types";
...
export default function CasinoArticlesPage() {
  const { user } = useAuth();
  const [page, setPage] = useState(0);
  const [status, setStatus] = useState<string>("");
  const [search, setSearch] = useState("");
  const { data, isLoading } = useCasinoArticlesList({
    page: page + 1,
    limit: PAGE_SIZE,
    status: status || undefined,
    q: search || undefined,
  });
  const deleteArticle = useDeleteCasinoArticle();
  const [deletingId, setDeletingId] = useState<string | null>(null);
  const [deleteConfirm, setDeleteConfirm] = useState<{ id: string; title: string } | null>(null);

  const handleDeleteClick = (article: CasinoArticle) => {
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      renderCell: (params) => (
        <Box>
          <IconButton size="small" component={Link} href={`/casino-articles/${params.row.slug}`} aria-label="View">
            <VisibilityIcon fontSize="small" />
          </IconButton>
          {canEditOwnedContent(user?.role, user?.id, params.row.createdBy?.id) && (
            <>
              <IconButton size="small" component={Link} href={`/casino-articles/${params.row.slug}/edit`} aria-label="Edit">
  ...
  return (
    <Box sx={{ width: "100%", minWidth: 0 }}>
      <PageHeader
        ...
        subtitle="Manage casino articles and content."
        action={<Button component={Link} href="/casino-articles/new" variant="contained" color="primary" startIcon={<AddIcon />}>Add Article</Button>}
      />
