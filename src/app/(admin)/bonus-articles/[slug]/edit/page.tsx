...
import {
  useBonusArticlesList,
  useBonusArticle,
  useUpdateBonusArticle,
} from "@/hooks/useBonusArticles";
...
export default function EditBonusArticlePage() {
  const params = useParams();
  const router = useRouter();
  const slug = params.slug as string;
  const { data: listData, isLoading: listLoading } = useBonusArticlesList({
    q: slug,
    limit: 50,
  });
  ...
  const { data: article, isLoading: articleLoading } = useBonusArticle(id);
  const update = useUpdateBonusArticle(article?.id ?? "");
  ...
  useEffect(() => {
    ...
    reset({
      ...
      bonusId: article.bonusId ?? article.bonus?.id ?? "",
    });
  }, [article, reset]);
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      if (values.featureImg) body.featureImg = values.featureImg;
      if (content !== undefined) body.content = content;
      if (values.tags?.length) body.tags = values.tags;
      if (values.gameSlugs?.length) body.gameSlugs = values.gameSlugs;
      if (values.seoTitle) body.seoTitle = values.seoTitle;
      if (values.seoDesc) body.seoDesc = values.seoDesc;
      if (values.focusKeywords?.length)
        body.focusKeywords = values.focusKeywords;
      body.showInBlog = values.showInBlog;
      body.showInGameArticle = values.showInGameArticle;
      body.showInBonusArticle = values.showInBonusArticle;
      body.showInCasinoArticle = values.showInCasinoArticle;
      if (values.bonusId) body.bonusId = values.bonusId;
      await update.mutateAsync(body);
      router.push("/bonus-articles");
    } catch (err: unknown) {
  ...
  if (!listLoading && listData && articleBySlug == null) {
    return (
      <Box>
        <Button component={Link} href="/bonus-articles" sx={{ mb: 2 }}>
          Back
        </Button>
  ...
  return (
    <ArticleEditorLayout
      title="Edit Bonus Article"
      subtitle={subtitle}
      backHref={`/bonus-articles/${article.slug}`}
      backLabel="Back to Article"
      submitLabel="Save changes"
      cancelHref={`/bonus-articles/${article.slug}`}
      ...
      rightChildren={
        <>
          <EditorCard title="Link to bonus">
            <Controller
              name="bonusId"
              control={control}
              render={({ field }) => (
            ...
            >
              Optionally link this article to a bonus.
            </Typography>
          </EditorCard>
