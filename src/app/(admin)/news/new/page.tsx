...
export default function NewNewsPage() {
  ...
  const { register, control, handleSubmit, setError, watch, setValue, formState: { errors, isDirty } } = useForm<FormValues>({
    resolver: zodResolver(schema),
    defaultValues: {
      title: "", slug: "", shortDesc: "", publishDate: new Date().toISOString().slice(0, 10), readTime: "5 min read",
      featureImg: "", content: "", tags: [], seoTitle: "", seoDesc: "", focusKeywords: [], isTrending: false, status: "draft",
    },
  });
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      await create.mutateAsync({
        title: values.title, slug, shortDesc: values.shortDesc,
      ...
      if (code === "SLUG_EXISTS") setError("title", { message: "A news item with this title (slug) already exists. Try a different title." });
      else setError("root", { message: getDisplayErrorMessage(err, "Failed to create") });
    }
  };
  ...
  return (
    <ArticleEditorLayout
      ...
      subtitle="Fill in the details to publish a new news or update."
      backHref="/news"
      ...
      leftChildren={
        <>
          ...
          <EditorCard title="Short Description">
            <TextField fullWidth placeholder="Write a brief summary that appears in news listings..." {...register("shortDesc")} multiline rows={3} size="small" />
            <Box component="span" sx={{ display: "block", mt: 0.5, fontSize: "0.75rem", color: "text.secondary" }}>This will be shown as the excerpt in news cards</Box>
          </EditorCard>
          ...
          <SeoFieldsCard
            ...
            focusKeywordsNode={<Controller name="focusKeywords" control={control} render={({ field }) => <ChipsInput value={field.value} onChange={field.onChange} placeholder="e.g. update, announcement" />} />}
          />
        </>
      }
      rightChildren={
        <>
          ...
          <EditorCard title="Featured Image">
            <Controller name="featureImg" control={control} render={({ field }) => <ImageUploadField label="Image URL" value={field.value ?? ""} onChange={field.onChange} previewAlt="News" helperText="PNG, JPG, GIF up to 5MB" />} />
          </EditorCard>
